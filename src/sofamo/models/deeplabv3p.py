...
from .deeplabv3p_backbones import ResNet, Xception
...
class DeepLabV3p(nn.Module):
    def __init__(self, out_channels, in_channels=3, backbone='resnet101', pretrained=True,
        ...
        if 'resnet' in backbone:
            self.backbone = ResNet(
                in_channels=in_channels, output_stride=output_stride, pretrained=pretrained)
            low_level_channels = 256
        else:
            self.backbone = Xception(
                output_stride=output_stride, pretrained=pretrained)
            low_level_channels = 128

        self.ASSP = ASSP(in_channels=2048, output_stride=output_stride)
        self.decoder = Decoder(low_level_channels, out_channels)
