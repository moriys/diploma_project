...
class WHUBuildingsDataset(torch.utils.data.Dataset):
    """WHUBuildings Dataset"""
    DFLT_CLASSES = {
        "background": rgb(0, 0, 0),
        "buildings": rgb(255, 255, 255),
    }
...
# %% 
if __name__ == "__main__":
    path_root = Path("/home/sofa/space/prog/data/WHU_buildings_PNGs/train_limited/")
    path_img = path_root / 'image'
    path_mask = path_root / 'label'
    fns_images = sorted(list(path_img.glob("*.png")))
    ...
    train_dataset = WHUBuildingsDataset(
        filenames=filenames,
