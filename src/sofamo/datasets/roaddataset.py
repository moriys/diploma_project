...
def intersection_filenames(flist0, flist1):
    ...
    for fn0, fn1 in product(flist0, flist1):
        if fn0.stem.split('_')[0] == fn1.stem.split('_')[0]:
...
# %% ksdjahfasldfksadf'lh
if __name__ == "__main__":
    path_root = Path("/data/data.sets/free-mlspace/DeepGlobe_Road/train0/")
    fns_images = sorted(list(path_root.glob("*_sat.*")))
    fns_masks = sorted(list(path_root.glob("*_mask.")))
